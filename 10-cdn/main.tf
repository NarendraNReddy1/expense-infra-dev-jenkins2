resource "aws_cloudfront_distribution" "web_cdn" {
  origin {
    domain_name              =  "web-${var.environment}.${var.zone_name}"#web-dev.narendra.shop
  ...
  aliases = ["web-${var.common_tags.Component}.${var.zone_name}"]#web-cdn.narendra.shop
...
module "records" {
  ...
  records = [
    {
      name    = "web-cdn" #web-cdn.narendra.shop
      type    = "A"
